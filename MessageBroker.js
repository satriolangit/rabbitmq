...
class MessageBroker {
  ...
  async init() {
    this.connection = await amqp.connect(
      process.env.RABBITMQ_URL || "amqp://root:wahyu123@localhost"
    );
   ...
   * @param {JSONObject} msg Message as Buffer
   */
  ...
  /**
   * Broadcast message to exchage using fanout mode
   * @param {String} exchange Exchange name
   * @param {Object} msg Message as Buffer
   */
  async broadcast(exchange, msg) {
    if (!this.connection) {
      await this.init();
    }
   ...
   */
  async subscribeToExchange(exchange, queue, handler) {
    if (!this.connection) {
      await this.init();
    }
